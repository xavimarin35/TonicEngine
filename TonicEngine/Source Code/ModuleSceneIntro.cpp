...
GameObject* ModuleSceneIntro::MousePicking(const LineSegment& segment, float& distance, bool close) const
{
	distance = FLOAT_INF;

	GameObject* picked = nullptr;

	std::map<float, const GameObject*> objs;
	std::vector<const GameObject*> root;

	float hit, hit2;

	GO::FillChildren(root, GOroot);

	for (int i = 0; i < root.size(); i++)
	{
		if (root[i]->data.active && segment.Intersects(root[i]->aabb, hit, hit2))
		{
			if (close)
				objs[hit] = root[i];
			else
	...
	for (std::map<float, const GameObject*>::const_iterator it = objs.begin(); it != objs.end(); it = next(it))
	{
		...
		if (mesh != nullptr)
		{
			...
			seg.Transform(transf->GetGlobalTransform().Inverted());

			Triangle triangle;

			for (uint i = 0; i < mesh->mData.num_index; i += 3)
			{
				triangle.a = mesh->mData.vertex[mesh->mData.index[i]];
				triangle.b = mesh->mData.vertex[mesh->mData.index[i + 1]];
				triangle.c = mesh->mData.vertex[mesh->mData.index[i + 2]];

				float dist; float3 hit;

				if (seg.Intersects(triangle, &dist, &hit))
				{
					if (close && dist < distance || !close && dist > distance)
					{
						distance = dist;
